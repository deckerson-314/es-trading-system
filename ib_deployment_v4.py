...
def get_front_es_contract():
    for attempt in range(3):
        try:
            ...
            if not future_cds:
                # Fallback to absolute front if no future contracts meeting criteria
                front_details = sorted(cds, key=lambda cd: datetime.strptime(cd.contract.lastTradeDateOrContractMonth, '%Y%m%d'))[0]
                front = front_details
            else:
