...
class TestExitLogic(unittest.TestCase):
    ...
    def test_short_exit_trigger(self):
        """Test that a short trade DOES trigger exit when strategy says so."""
        strategy = MagicMock()
        ...
        old_force = core.execution._force_close_position
        core.execution._force_close_position = MagicMock()
        
        try:
            check_exits(strategy, ib, contract, data, positions, [],
                        None, MagicMock(), 0, latest_row, allow_strategy_exit=True)
            core.execution._force_close_position.assert_called_once()
        finally:
            core.execution._force_close_position = old_force
    ...
    def test_check_exits_clamping_for_entry_bar(self):
        ...
        # Call check_exits with allow_strategy_exit=True
        # This simulates the call in on_bar_update_handler for the resampled bar
