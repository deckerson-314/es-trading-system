...
def debug_trend_filter():
    ...
    # Define runs
    runs = [
        {'name': 'BASELINE (No Filter)', 'filter': 0, 'len': 200},
        {'name': 'TEST (Trend Filter ON)', 'filter': 1, 'len': 200}
    ]
    ...
    for run in runs:
        ...
        params_flat['Enable Trend Filter'] = run['filter']
        params_flat['Trend EMA Length'] = run['len']
    ...
    if results['test_long'] < results['base_long'] and results['test_short'] < results['base_short']:
        print("PASS: Trend Filter successfully reduced signals.")
