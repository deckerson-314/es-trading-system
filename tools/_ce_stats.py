...
def main():
    hdr("Jun-21 GA with channel exit lookbacks")
    _, nc, nb = load(NEW)
    _, oc, ob = load(OLD)
    ns, os = ex(nb, nc), ex(ob, oc)
    agg_metrics("Jun-21", ns)
    channel_report("Jun-21 all", ns)
    pos=[s for s in ns if s.oos_pnl and s.oos_pnl>0]
    if pos: channel_report("Jun-21 OOS-positive", pos)
    hdr("Jun-21 vs Jun-20 headline")
    agg_metrics("Jun-20", os)
    agg_metrics("Jun-21", ns)
    hdr("Top 15 Jun-21 by OOS PnL")
