...
NEW=Path('C:/Trading/Trend/parameters/genetic_results_2026-07-02-1.csv')
OLD=Path('C:/Trading/Trend/parameters/genetic_results_2026-06-23-1.csv')
REF=Path('C:/Trading/Trend/parameters/genetic_results_2026-06-21-1.csv')
...
def main():
    hdr("Jul-02 GA (latest) with paper-fidelity sim")
    _, nc, nb = load(NEW)
    _, oc, ob = load(OLD)
    _, rc, rb = load(REF)
    ns, os, rs = ex(nb, nc), ex(ob, oc), ex(rb, rc)
    agg_metrics("Jul-02", ns)
    agg_metrics("Jun-23", os)
    agg_metrics("Jun-21", rs)
    channel_report("Jul-02 all", ns)
    pos=[s for s in ns if s.oos_pnl and s.oos_pnl>0]
    print(f"\n--- Jul-02 OOS-positive: {len(pos)} / {len(ns)} ---")
    if pos:
        channel_report("Jul-02 OOS-positive", pos)
    hdr("Top 15 Jul-02 by OOS PnL")
    for s in sorted(ns,key=lambda s:s.oos_pnl or -1e99,reverse=True)[:15]:
        print("  "+line(s))
    best("Jul-02", ns)
    deploy("Jul-02", ns)
    # fidelity template rows
    hdr("Fidelity flags (template Value in Jul-02 CSV)")
    for name in (
        "GA_LIVE_STYLE_ENTRY", "GA_PESSIMISTIC_STOPS",
        "GA_CONSERVATIVE_STOP_SLIPPAGE", "GA_CONSERVATIVE_ENTRY_SLIPPAGE",
        "GA_CONSERVATIVE_CHANNEL_SLIPPAGE", "Enable Trailing Stop",
        "Enable Maintenance Filter",
    ):
        r = nb.get(name)
        if r:
            print(f"  {name}: Value={r.get('Value')} Min={r.get('Min')} Max={r.get('Max')}")
    # distribution snapshots
    hdr("Jul-02 gene distributions")
    for attr, lab in [
        ("trail_delay", "trail_delay"), ("timeframe", "TF"),
        ("ce_sell_lb", "CE sell"), ("ce_buy_lb", "CE buy"),
        ("ce_atr_offset", "CE off"), ("tp_atr", "TP ATR"),
    ]:
        vals=[getattr(s,attr) for s in ns if getattr(s,attr) is not None]
        if not vals: continue
        c=Counter(int(v) if attr!="ce_atr_offset" and attr!="tp_atr" else round(v,2) for v in vals)
        print(f"  {lab}: modes={c.most_common(5)}")
    trail_on=sum(1 for s in ns if s.enable_trailing==1)
    print(f"  Enable Trailing Stop ON: {trail_on}/{len(ns)}")
    maint_on=sum(1 for s in ns if s.enable_maint==1)
    print(f"  Enable Maintenance Filter ON: {maint_on}/{len(ns)}")
