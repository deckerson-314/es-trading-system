...
graph TD
    ...
    subgraph Verification["Verification Suite"]
        ...
        Attribution["strategy_attribution.py<br/>(Four-Quadrant Entry/Exit)"]:::script
        ...
        Attribution -->|Read| BTTrades
        Attribution -->|Read| HistData
        Attribution -->|Core| TradeAttrib[("core/trade_attribution.py")]:::logic
        Attribution -->|Generate| AttrReport[("results/attribution_*.md")]:::output
    end
...
5.  **Strategy Attribution (`tools/analysis/strategy_attribution.py`):** Diagnostic layer for **structural edge**. Decomposes OOS trades into four quadrants (strategy vs random entry × strategy vs random exit), computes **MFE/MAE** excursion metrics, and compares against a Monte Carlo random-trading null. Use before trusting GA OOS or paper deploy. See `docs/strategy_attribution_analysis.md`.
6.  **Session Strategy (`strategies/session/`):** Active intraday ES research target — VWAP mean reversion with opening-range regime. GA: `python optimize.py --strategy session --fresh`. See `docs/strategy_pivot.md`. Trend/Donchian is **deprecated**.

## Directory Structure & Default File Locations

Below are the default paths used by the system if custom paths are not provided during execution:

### 1. Market Data
*   **Historical Data Directory:** `c:\Trading\data\`
*   **Live Tick Data Output:** `c:\Trading\live_data.csv`
*   **Live Execution Trade Log:** `c:\Trading\live_trades.csv`

### 2. Strategy Configurations
*   **GA Optimized Parameters:** `c:\Trading\live_params.csv`
*   **GA All Solutions Pool:** `c:\Trading\solutions.json`
*   **Parameter Analysis Output:** `c:\Trading\web\parameter_analysis\`

### 3. Dashboards & Web Outputs
By design, all generated HTML dashboards and output interfaces are saved directly to the **`c:\Trading\web\`** directory so they can be securely hosted and accessed externally.

*   **Primary Index:** `web/index.html` (Serves as the root directory to access all other dashboards)
*   **Live Trading Dashboard:** `web/dashboard_live.html`
*   **Paper Trading Dashboard:** `web/dashboard_paper.html`
*   **Backtest Results Dashboard:** `web/backtest_dashboard_bollinger.html` (By default, generated by `backtest.py`)
*   **Genetic Algorithm Dashboard:** `web/ga_dashboard_v4.html`
*   **Comparison Overlays & Auditor Charts:** `web/comparison_charts/`
*   **Strategy Attribution Reports:** `results/attribution_*.md` (generated by `tools/analysis/strategy_attribution.py`)

### 4. Diagnostics & Attribution
*   **Attribution CLI:** `tools/analysis/strategy_attribution.py`
*   **Attribution Library:** `core/trade_attribution.py`
*   **Documentation:** `docs/strategy_attribution_analysis.md`
*   **Legacy wrappers:** `tools/random_trades_baseline.py`, `tools/entry_edge_study.py`

*Note: Access `index.html` via a local or external web server to traverse the generated dashboards and active status views.*
